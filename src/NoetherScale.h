 ...
 * the left, one octave per step: 1/4 1/2 1 2 4. The marker is the effective
...
public:
    ...
    // octave from 1/4 (0.2) to 4 (1.0); linear below 1/4.
    static inline float place(float a)
    {
        if (a <= 0.25f) return 0.2f * (a / 0.25f);
        float u = 0.2f;
        while (a > 0.25f * 1.0001f && u < 1.0f) { a *= 0.5f; u += 0.2f; }
        // a is now in (0.125, 0.25]; interpolate the last partial octave
        // by the fraction of an octave left: log2(a / 0.125) in [0,1)
        const float frac = (a - 0.125f) / 0.125f;   // linear within the octave: close enough for 11 px
        return u - 0.2f + 0.2f * frac;
    }
    ...
    virtual void draw(od::FrameBuffer &fb)
    {
        ...
        static const char *kLabels[5] = { "1/4", "1/2", "1", "2", "4" };
        for (int i = 0; i < 5; ++i) {
            const float u = 0.2f * (float)(i + 1);
            const int dx = (int)(u * (float)half + 0.5f);
            fb.vline(GRAY7, cx + dx, yAxis - 2, yAxis + 2);
            fb.vline(GRAY7, cx - dx, yAxis - 2, yAxis + 2);
            const int lw = (i == 2 || i == 4) ? 3 : 9;
        ...
        if (a < 0.001f) {
            ...
            if (h->getDetent()) {
                const char *lab = a >= 3.0f ? "4x" : a >= 1.5f ? "2x" : a >= 0.75f ? "1x" : a >= 0.375f ? "1/2" : "1/4";
        ...
        if (L > 0) {
            ...
            snprintf(buf, sizeof buf, "%.2f s   %d/%d", (double)h->getLoopSeconds(), cur, sec);
            fb.text(GRAY9, x0 + 4, y0 + 4, buf, 10);
        } else {
            fb.text(GRAY7, x0 + 4, y0 + 4, "no loop", 10);
        }
    }
